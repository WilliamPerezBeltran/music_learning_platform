      ...
      <div class="flex flex-col md:flex-row min-h-dvh text-white" style="background:#07070e;">
        ...
        >
          ...
          <div class="px-5 py-5" style="border-bottom:1px solid rgba(255,255,255,0.06);">
            <a href="/" class="flex items-center gap-2.5 group">
              <div
                ...
                style="background:linear-gradient(135deg,#E53935,#E91E63);"
              >
                🎹
              </div>
              <span class="font-bold text-sm text-white group-hover:opacity-80 transition-opacity">
                PianoColor
              </span>
            </a>
          </div>
          ...
          <div class="flex items-center justify-between px-5 pt-5 pb-3">
            ...
            <button
              class="md:hidden w-7 h-7 rounded-lg flex items-center justify-center text-white hover:opacity-70 transition-opacity"
          ...
          <div class="flex-1 overflow-y-auto px-3 pb-4 space-y-0.5">
            <%= for song <- @songs do %>
              ...
              >
                <div class="flex items-center gap-2.5">
                  <span
                    class="w-5 h-5 rounded-md flex items-center justify-center text-[10px] flex-shrink-0"
            ...
            <%= if @songs == [] do %>
              <p class="text-xs text-white/60 px-3 py-2">
                No hay canciones. Ejecuta
                <code class="text-white/80">mix run priv/repo/seeds.exs</code>
              </p>
            <% end %>
          </div>
          ...
          <div class="px-5 py-4" style="border-top:1px solid rgba(255,255,255,0.06);">
            ...
            <div class="flex gap-1.5">
              <%= for {note, hex} <- [{"Do","#E53935"},{"Re","#FB8C00"},{"Mi","#FDD835"},{"Fa","#43A047"},{"Sol","#1E88E5"},{"La","#8E24AA"},{"Si","#E91E63"}] do %>
                <div title={note} class="flex-1 h-1.5 rounded-full" style={"background:#{hex};"}>
                </div>
              <% end %>
            </div>
          </div>
        ...
        <main class="flex-1 flex flex-col min-w-0" style="background:#0b0b17;">
          <%!-- Header --%>
          <div
            class="shrink-0 px-4 sm:px-6 py-3.5 flex items-center gap-3"
          ...
          >
                ...
                <div class="flex gap-1.5 ml-1">
                  <%= for v <- @versions do %>
                    <button
                      ...
                      phx-value-id={v.id}
                      class={[
                        "px-3 py-1 text-[11px] font-bold transition-all ctrl-glass",
            ...
            <a
              href="/"
              class="ml-auto flex items-center gap-1.5 text-xs text-white/60 hover:text-white transition-colors hidden sm:flex"
            >
              <.icon name="hero-arrow-left" class="w-3 h-3" /> Inicio
            </a>
          </div>
          ...
          <%= if @selected_song && @timeline_loaded do %>
            ...
            >
              <%= if @is_playing do %>
                <button
                  phx-click="pause"
                  class="w-9 h-9 rounded-full flex items-center justify-center text-white ctrl-glass"
                  aria-label="Pausar"
                >
              ...
              <% else %>
                <button
                  phx-click="play"
                  class="w-9 h-9 rounded-full flex items-center justify-center text-white play-btn"
              ...
              <div class="ml-auto flex items-center gap-2">
                <button
                  ...
                  class={[
                    "w-8 h-8 rounded-full flex items-center justify-center ctrl-glass",
                    if(@colors_enabled, do: "active", else: "")
                  ]}
          ...
          <div class="flex-1 overflow-auto p-4 sm:p-6">
                    ...
                    >
                      <div class="flex items-start justify-between gap-2">
                        <div
                          ...
                          style={"#{if @selected_song && @selected_song.id == song.id, do: "background:linear-gradient(135deg,#E53935,#E91E63);", else: "background:rgba(255,255,255,0.07);"}"}
                        >
                          🎵
                        </div>
                      ...
                      <div class="flex gap-1 mt-auto">
                        <%= for {_n, hex} <- [{"Do","#E53935"},{"Re","#FB8C00"},{"Mi","#FDD835"},{"Fa","#43A047"},{"Sol","#1E88E5"},{"La","#8E24AA"},{"Si","#E91E63"}] do %>
                          <div class="flex-1 h-1 rounded-full opacity-40" style={"background:#{hex};"}>
                          </div>
                        <% end %>
                      </div>
                    </button>
            ...
            <%= if @selected_song do %>
              <div>
                ...
                <div
                  class="rounded-2xl overflow-hidden shadow-2xl shadow-black/50"
